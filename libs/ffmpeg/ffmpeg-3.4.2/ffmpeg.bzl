...
load("@bazel_machinery//libs/ffmpeg/ffmpeg-3.4.2:makefile.bzl", "makefile_parse", "format_makefile")
...
def ffmpeg_repository(index, name):
    info = index["ffmpeg"]
    urls = info.get("urls")
    sha256 = info.get("sha256")
    strip_prefix = ""
    if "strip_prefix" in info:
    ...
    _ffmpeg_repository(
        ...
        name = name,
        build_file_template = Label(":ffmpeg.BUILD"),
        config_h = Label(":config.h"),
        patches = [
            # From: https://patchwork.ffmpeg.org/patch/8253/
            Label(":using_dn_directive.diff"),
        ],
    )
